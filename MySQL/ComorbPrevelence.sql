...
select merged_comorb_name, count(1)  c from (select distinct admid, case 
    ...
    merged_comorb_name from base_data  
 ) x 
 where merged_comorb_name not in ('Endotracheally intubated','On mechanically assisted ventilation',
 'Tracheostomy in place','Palliative care by specialist')
